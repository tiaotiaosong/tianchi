...
class SHDW(object):
    ...
    def generate_train_data(self, data):
        ...
        data = data[data['shop_id'].isin(shop_ans)]
        ...
        wifi_ans = ['long','lati']+list(set(wifi_tmp + wifi_fliter))
        ...
        # X = clf.predict_proba(test_x[:, 2:])
        clf.fit(train_x, train_y)
        X = clf.predict_proba(test_x)

        clf1 = RandomForestClassifier(n_estimators=100)
        clf1.fit(train_x[:,:2], train_y)
        X1=clf1.predict_proba(test_x[:,:2])

        # y_1 = np.argmax(X, axis=1)
        # print y_1
        # y_2 = clf.predict(test_x[:, 2:])
        # print y_2

        XX = 0.96*X+0.04*X1
        y_X = np.argmax(XX, axis=1)
        print len(y_X)


        # print clf.score(test_x[:, 2:],test_y)
        print clf.score(test_x, test_y)
        print clf1.score(test_x[:,:2],test_y)
