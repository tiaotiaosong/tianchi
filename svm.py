...
class SHDW(object):
    ...
    def generate_train_data(self, data):
        ...
        for index, data in df.iterrows():
            bssid = data[0]
            row = int(data[2])
            strength = max(float(data[3]),-80)
            strength = (strength+80)/60
        ...
        train_x, test_x, train_y, test_y = train_test_split(data_tmp.values, y, test_size=0.02, random_state=1)
        # # # #
        # del data_tmp
        # gc.collect()
        ...
        gsearch1 = svm.SVC(decision_function_shape='ovr', C=1.1, gamma=0.00005)
        gsearch1.fit(train_x, train_y)
            # svm.SVC(decision_function_shape='ovr', C=1,gamma =0.0001)  score 0.45
            # svm.SVC(decision_function_shape='ovr', C=1, gamma=0.00007) score 0.556962025316
            # svm.SVC(decision_function_shape='ovr', C=1,gamma =0.00006) score 0.584810126582
            # svm.SVC(decision_function_shape='ovr', C=1, gamma=0.000055) score 0.589873417722
            # svm.SVC(decision_function_shape='ovr', C=1,gamma =0.00005) score 0.6025
            # svm.SVC(decision_function_shape='ovr', C=1,gamma =0.000045) score 0.59746835443
            # svm.SVC(decision_function_shape='ovr', C=1,gamma =0.00004) score 0.592405063291


        print gsearch1.score(train_x, train_y)
        print gsearch1.score(test_x, test_y)
